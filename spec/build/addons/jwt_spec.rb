...
describe Travis::Build::Addons::Jwt, :sexp do
  ...
  subject      { sh.to_sexp }
  before       { Time.stubs(:now).returns(Time.utc(1970,1,1)) }
  before       { addon.before_before_script }

  describe 'jwt token, one secret' do
    let(:config) { 'MY_ACCESS_KEY=987654321' }
    it "should work" do
      should include_sexp [:echo, 'Initializing JWT', ansi: :yellow]
      ...
      should include_sexp [:export, ['MY_ACCESS_KEY', expected]]
    end
  end
  ...
  describe 'jwt token, several secrets' do
    ...
    it {
      ...
      should include_sexp [:export, ['MY_ACCESS_KEY_1', expected1]]
      ...
      should include_sexp [:export, ['MY_ACCESS_KEY_2', expected2]]
    }
  end
end
